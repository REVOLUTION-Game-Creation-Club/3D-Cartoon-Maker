...
LmScene& LmScene::operator=( LmScene& _rhs )
{
	...
	if(this != &_rhs)
	{
		...
		{
			wstring wstrName = itrChar->second->GetName();
			LmChar* pNewChar = new LmChar(wstrName.c_str());
			*pNewChar = *(itrChar->second);
			m_mapChar[wstrName] = pNewChar;
		}
...
VOID LmScene::SetFrameSize( UINT _uFrameSize )
{
	m_uFrameSize = _uFrameSize;
	m_listInfoCamera.resize(_uFrameSize+1);
	m_listInfoFirework.resize(_uFrameSize+1);
