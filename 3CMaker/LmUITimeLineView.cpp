...
//	TheCartoonMaker->InitTimeLIne();
	...
	m_pSceneView = (LmUISceneView*) pMainFrame->m_wndSplitUpColExpand.GetPane(0,0);
...
INT LmUITimeLineView::GoNextCaret()
{
	...
	else
	{
		...
		//INT nResult = m_pSceneView->GetNextThumbNail(strSceneName);
		//if(!nResult)
		//	//이상없으면 다음 씬으로 이동한다
		//{
		//	TheCartoonMaker->SelectNowScene(strSceneName.GetBuffer(0));
		//}
		//else
		//카메라를 마우스로 조작할 수 있음.
		//TheCartoonMaker->EnableCameraMouse(TRUE);
